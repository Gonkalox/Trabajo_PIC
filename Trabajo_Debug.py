...
class DrumApp:
    ...
    def run(self):
        while True:
            ...
            if key == ord('w'):
                ...
                # Ensure ROI exists before forcing CONFIG
                if self.roi is None:
                    h, w = frame.shape[:2]
            ...
            if self.state == STATE_WAITING:
                ...
                # Pass the override value specifically for Config local processing
                self._handle_config(frame, display_frame, stick_pos_global, override_fingers=self.simulated_fingers)

            elif self.state == STATE_PLAYING:
                self._handle_playing(frame, display_frame, effective_fingers_global, stick_pos_global)
    ...
    def _handle_config(self, frame, display, stick_pos, override_fingers=0):
